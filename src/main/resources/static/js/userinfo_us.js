...
var TableInit = function () {
    ...
    oTableInit.queryParams = function (params) {
        var temp = {   //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
            ...
            name: $(".code").val(),
            status: $("#level_box").val(),
        };
        return {"name":temp.name,"size":temp.size,"page":temp.page,"status":temp.status};
    };
    return oTableInit;
};
  ...
  return oInit;
    ...
    function operateFormatter(value, row, index) {
            ...
            return [
                ...
                '<button type="button" class="RoleOfD btn btn-default  btn-link" style="margin-right:15px;">查看角色下用户</button>'
            ].join('');
        }
